#!/usr/bin/env python3
"""Prove the frozen Save Reader seam and run its focused guest oracle."""
...
ORACLE_RESULT = (
    "Vita Save Reader guest oracle: PASS; checks=203; "
...
def verify_codegen(pe_path: Path) -> None:
    ...
    for base in (DEFAULT_BASE, 0x98000000):
        ...
        require(result["stub"] is None,
                f"Save Reader target became a stub at base {base:#x}")
        require(result["vita_save_reader_fastpath"],
                f"Save Reader proof was not selected at base {base:#x}")
        text = result["text"]
        require(text.count("isaac_vita_save_reader_guest_try(c)") == 1,
                "Save Reader entry seam count changed")
        require("defined(ISAAC_VITA_SAVE_READER_FASTPATH)" in text,
                "Save Reader feature fence disappeared")
        require("/* 0025ba60  push ebp */" in text and
                "/* 0025baa0  ret 0xc */" in text,
                "translated Save Reader fallback disappeared")


def run_oracle(cc: str) -> str:
    with tempfile.TemporaryDirectory(prefix="isaac-save-reader-") as value:
        ...
        run([
            ...
            "-Wextra",
            "-Werror",
            "-DISAAC_VITA_SAVE_READER_FASTPATH=1",
            "-DISAAC_VITA_PHASE_PROFILE=1",
            "-DISAAC_VITA_MEMORY_RANGE_GUARD=1",
            "-DISAAC_VITA_MEMORY_RANGE_ORACLE=1",
            "-I",
            str(RUNTIME),
            str(RUNTIME / "host_vita_memory.c"),
            str(RUNTIME / "host_vita_save_reader_guest.c"),
            str(RUNTIME / "host_vita_save_reader_guest_oracle.c"),
            "-o",
            str(output),
        ])
...
def main() -> int:
    ...
    require(result == ORACLE_RESULT, "Save Reader oracle result changed")
    print(result)
    print("Vita Save Reader fast path: PASS")
    return 0
