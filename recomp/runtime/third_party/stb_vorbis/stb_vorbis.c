...
#include <stdlib.h>
#include <string.h>
/* ISAAC (repentogxm) local patch: the only edit to upstream v1.04.  The
 * embedding runtime may route assert() to a non-aborting handler so a
 * corrupt stream can never abort() the Vita process from a worker thread.
 * Everything else in this file is byte-identical to upstream commit
 * b8e0530fdfbe (2014-08-28). */
#ifdef ISAAC_NV_ASSERT_HEADER
#include ISAAC_NV_ASSERT_HEADER
#else
#include <assert.h>
#endif
/* ISAAC (repentogxm) local patch 2: allocation seam.  The Vita runtime
 * compiles every translation unit with the raw libc allocators poisoned, so
 * the eight allocator call sites below go through these macros; without the
 * embedding runtime they are the upstream libc calls. */
#ifndef ISAAC_NV_MALLOC
...
static void *setup_malloc(vorb *f, int sz)
{
   ...
   return sz ? ISAAC_NV_MALLOC(sz) : NULL;
}
...
int stb_vorbis_decode_filename(const char *filename, int *channels, int *sample_rate, short **output)
{
   ...
   *channels = v->channels;
   ...
   offset = data_len = 0;
   total = limit;
   data = (short *) ISAAC_NV_MALLOC(total * sizeof(*data));
   if (data == NULL) {
      stb_vorbis_close(v);
      return -2;
   }
   for (;;) {
      ...
      if (offset + limit > total) {
         short *data2;
         total *= 2;
         data2 = (short *) ISAAC_NV_REALLOC(data, total * sizeof(*data));
         if (data2 == NULL) {
            ISAAC_NV_FREE(data);
            stb_vorbis_close(v);
            return -2;
